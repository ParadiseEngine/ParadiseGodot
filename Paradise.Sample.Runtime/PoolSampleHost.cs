...
/// <summary>Standalone host for the pool ImGui MVVM demo (<c>--game pool</c>). The UI runs on
/// <see cref="ImGuiSampleRunner"/>'s 60 Hz sim thread; this host only pumps SDL events into the
/// runner's queue and renders an empty <see cref="PbrScene"/> (a clear) composited with the ImGui
/// overlay through the renderer's OverlayPass. The ImGui frame itself is produced on the SIM thread;
/// the render half here just replays the latest snapshot. The demo core (<see cref="PoolSampleUi"/>)
/// lives with pool and is shared with the pool Godot bridge.</summary>
internal static class PoolSampleHost
{
    ...
    private static int RunHeadless(ImGuiSampleRunner runner, int frameCount, string? screenshotPath)
    {
        ...
        try
        {
            ...
            {
                Console.Error.WriteLine($"[PoolSample] sim thread faulted: {ex}");
                return 1;
            }
            Console.WriteLine($"[PoolSample] Headless: rendered {frameCount} frames, sim tick {runner.Frame}.");

            if (screenshotPath is not null)
            {
                var pixels = renderer.ReadbackColor(out var w, out var h);
                Program.WriteBmp(screenshotPath, pixels, w, h);
                Console.WriteLine($"[PoolSample] Screenshot: {screenshotPath} ({w}x{h}).");
            }
            return 0;
        }
        finally
    ...
    private static unsafe int RunWindowed(ImGuiSampleRunner runner)
    {
        ...
        try
        {
            window = SDL_CreateWindow("Paradise — Pool ImGui Sample", Width, Height, SDL_WindowFlags.SDL_WINDOW_RESIZABLE);
            if (window == null)
            {
    ...
    private static (ImGuiUi ImGui, PbrRenderer Pbr, PbrScene Scene, PoolSampleUi Sample) Compose(
        ImGuiSampleRunner runner, WebGpuRenderer renderer, uint width = Width, uint height = Height)
    {
        var imgui = new ImGuiUi(width, height);
        var sample = new PoolSampleUi();   // MVVM composition root: owns the snapshot sim
