...
/// <summary>
/// Owns the shared Paradise.ECS world and the ball-dynamics schedule, and advances them each tick.
/// It deliberately does NOT wrap entity/component access — callers (Godot, or the engine runtime)
/// use <see cref="World"/> directly: <c>sim.World.CreateEntity(...)</c>,
/// <c>sim.World.GetComponent&lt;LocalTransform&gt;(entity)</c>, and raw <see cref="Entity"/> handles.
/// This type exists only because the source-generated world/schedule generic types can't be named in
/// a caller's field; it holds them and exposes the world.
...
public sealed class GameSimulation : IDisposable
{
    ...
    public GameSimulation(Paradise.Physics.CollisionWorld? collisionWorld = null)
    {
    ...
    /// snapshot, refresh the shared <see cref="SimulationContext"/>, then run the schedule in
    /// snapshot-read mode — <see cref="MovementSystem"/> resolves ball dynamics in one pass,
    /// writing every final transform.
    ...
    /// spawn (as <see cref="SimulationRunner.SpawnBall"/> does) or movement is a no-op.
    /// </summary>
