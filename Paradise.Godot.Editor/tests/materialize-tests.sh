...
echo "$OUT" | grep -q "installing res:// payload 0.13.0" && ok "logs install" 1 || ok "logs install" 0
...
[ "$(cat "$ADDON/.paradise-addon-version" 2>/dev/null)" = "0.13.0" ] && ok "marker written = 0.13.0" 1 || ok "marker written = 0.13.0" 0
...
[ "$(cat "$ADDON/.paradise-addon-version")" = "0.14.0" ] && ok "marker bumped" 1 || ok "marker bumped" 0
...
[ "$(cat "$BADDON/.paradise-addon-version")" = "0.13.0" ] && ok "adopt writes marker" 1 || ok "adopt writes marker" 0
...
<Project Sdk="Microsoft.NET.Sdk">
  ...
  <ItemGroup><PackageReference Include="Paradise.Godot.Editor" Version="0.13.0" /></ItemGroup>
</Project>
EOF
