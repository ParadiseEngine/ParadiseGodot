...
namespace ParadiseGodot.Export
{
    ...
    internal sealed class MeshGlbExporter
    {
        ...
        /// <summary>Export every NON-entity visual (MeshInstance3D without an EntityExport
        /// ancestor) into one WORLD-space GLB — the scene's environment (floor, walls, …).
        /// Returns the contract field or null when the scene has no such visuals.</summary>
        public string? ExportEnvironment(Node root, string sceneName)
        {
            var meshes = new List<MeshInstance3D>();
            CollectEnvironmentMeshes(root, meshes);
            if (meshes.Count == 0)
            {
                return null;
            }

            string field = $"meshes/{sceneName}.environment.glb";
            string outputPath = _paths.GetMeshOutputPath(field);
            Directory.CreateDirectory(Path.GetDirectoryName(outputPath)!);

            // Rebuild the visuals under one identity root with their WORLD transforms — the
            // runtime places the environment instance at identity.
            var container = new Node3D { Name = "Environment" };
            try
            {
                foreach (MeshInstance3D mesh in meshes)
                {
                    var clone = (MeshInstance3D)mesh.Duplicate();
                    // Visual-only export: drop any cloned children.
                    foreach (Node child in clone.GetChildren())
                    {
                        clone.RemoveChild(child);
                        child.Free();
                    }
                    container.AddChild(clone);
                    // Container root is identity and the export tree is detached, so the local
                    // transform IS the world transform (GlobalTransform is tree-dependent).
                    clone.Transform = mesh.GlobalTransform;
                    clone.Owner = container;
                }

                var document = new GltfDocument();
                var state = new GltfState();
                Error error = document.AppendFromScene(container, state);
                if (error != Error.Ok)
                {
                    throw new IOException($"GltfDocument.AppendFromScene failed for the environment: {error}.");
                }

                File.WriteAllBytes(outputPath, document.GenerateBuffer(state));
            }
            finally
            {
                container.Free();
        ...
        /// <summary>Non-entity meshes only: anything under an EntityExport exports through that
        /// entity's Renderable instead.</summary>
        private static void CollectEnvironmentMeshes(Node node, List<MeshInstance3D> meshes)
        {
            if (node is EntityExport)
            {
                return;
            }
            ...
            {
                CollectEnvironmentMeshes(child, meshes);
            }
        }
